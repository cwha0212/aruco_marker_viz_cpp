...
//
// init_pose_from_marker_node 의 '검출 경로'를 그대로 재사용한다. 차이점:
//   - map pose / initialpose 없음. 대신 카메라 프레임 마커 pose 로부터 제어오차를 만든다.
//   - /cmd_vel(geometry_msgs/Twist) 로 속도명령을 발행해 로봇을 움직인다.
//
// 로봇은 4족보행(홀로노믹 평면): x(전진)·y(횡이동)·yaw 만 제어. 높이(z-up)는 제어 불가라
// 전부 지면 평면에 투영해 무시한다. 접근은 2단계:
//   1) ALIGN   : 전진 없이 중앙정렬(vy)+z축 정렬(yaw) 로 마커 정면 자세를 만든다.
//   2) APPROACH: 정면 유지한 채, 카메라 z거리(tvec[2]) 가 final_distance(0.10m) 될 때까지 직진.
// 제어기: 오차 P제어 → 속도 포화(max_linear_speed/max_yaw_rate) → 가속 제한(slew) → 데드밴드.
#include <algorithm>
#include <chrono>
#include <cmath>
#include <cstdio>
...
// 최신 마커 관측(제어에 쓰는 지면 투영 오차). 이미지 콜백이 갱신, 제어 타이머가 읽음.
struct Observation
{
  double z_cam{0.0};     // 카메라 z거리 = tvec[2] (m)
  double e_center{0.0};  // 중앙정렬 오차 = 바디 y (좌+) (m)
  double e_axis{0.0};    // z축 정렬 오차 = atan2(-ny,-nx) (rad)
  rclcpp::Time stamp;
  bool has{false};
};
...
public:
  ...
  {
    // --- 검출/캘리브 파라미터 (init_pose_from_marker_node 와 동일) ---
    ...
    // 근접 소실 대비 열린루프 마무리 + odom 진행거리 측정
    ...
    if (use_odom_) {
      sub_odom_ = create_subscription<nav_msgs::msg::Odometry>(
        odom_topic, rclcpp::QoS(10), std::bind(&MarkerApproachNode::onOdom, this, _1));
    }
...
private:
  // OPENLOOP: 근접 소실 시 마지막 구간을 무피드백(odom 진행거리/시간)으로 마무리
  ...
  // 로봇 localization(map 프레임 평면 위치). 열린루프 진행거리 측정에만 사용.
  ...
  void process(cv::Mat img)
  {
    ...
    auto markers = detectMarkers(*detector_, img, nullptr);
    ...
    for (const auto & [mid, corners] : markers) {
      ...
      // 마커 법선(z축)의 지면 투영 → z축 정렬 오차
      tf2::Vector3 n = R_body_marker.getColumn(2);  // 바디기준 마커 z축
      double raw_z = tvec[2];
      double raw_center = t_body_marker.y();
      double raw_axis = std::atan2(-n.y(), -n.x());  // 전방이 법선과 정반대일 때 0

      // EMA 저역통과(지터 완화). 소실 후 재획득이면 필터 리셋.
      auto t = now();
      double a = filter_alpha_;
      if (!filt_init_ || (t - filt_stamp_).seconds() > max_pose_age_) {
        f_z_ = raw_z; f_center_ = raw_center; f_axis_ = raw_axis;
        filt_init_ = true;
      } else {
        f_z_ = a * raw_z + (1.0 - a) * f_z_;
        f_center_ = a * raw_center + (1.0 - a) * f_center_;
        f_axis_ = wrapToPi(f_axis_ + a * wrapToPi(raw_axis - f_axis_));  // 각도 언랩 필터
      }
      filt_stamp_ = t;

      Observation o;
      o.z_cam = f_z_;
      o.e_center = f_center_;
      o.e_axis = f_axis_;
      o.stamp = t;
      o.has = true;
      {
  ...
  void onControl()
  {
    ...
    bool fresh = o.has && (now() - o.stamp).seconds() <= max_pose_age_;
    ...
    if (state_ == State::OPENLOOP) {
      if (fresh) {  // 재획득 → 폐루프 복귀
        ...
        if (done || now() >= ol_deadline_) {  // odom 도달 or 시간 안전상한
          publishStop();
          state_ = State::ARRIVED;
          RCLCPP_INFO(get_logger(),
            "열린루프 종료 → ARRIVED(추정 도달, 이동 %.3f/%.3fm, %s)",
            traveled, ol_remain_, done ? "odom" : "timeout");
          return;
        }
        publishSmoothed(openloop_speed_, 0.0, 0.0);  // 무피드백 저속 직진
        return;
      }
    }
    ...
    // --- 마커 소실 처리 ---
    if (!fresh) {
      // APPROACH 중 '가까이서' 소실 → 열린루프로 마지막 구간 마무리
      if (state_ == State::APPROACH && openloop_enabled_ && o.has &&
        o.z_cam <= openloop_enter_dist_)
      {
        ol_remain_ = std::clamp(o.z_cam - final_distance_, 0.0, openloop_max_dist_);
        if (ol_remain_ <= 1e-3) {
          publishStop();
          state_ = State::ARRIVED;
          RCLCPP_INFO(get_logger(), "소실 시 이미 목표거리 → ARRIVED (z=%.3f)", o.z_cam);
          return;
        }
        ol_use_odom_ = use_odom_ && odomFresh();
        ol_start_x_ = odom_x_; ol_start_y_ = odom_y_;
        // odom 모드는 시간을 안전상한(넉넉)으로, 시간 모드는 이동시간으로.
        ...
        have_prev_ = false;  // slew 초기화(정지→직진 부드럽게)
        state_ = State::OPENLOOP;
        RCLCPP_WARN(get_logger(),
          "근접 소실(z=%.3f) → 열린루프 %.3fm 직진(%.2f m/s, %s)",
          o.z_cam, ol_remain_, openloop_speed_, ol_use_odom_ ? "odom" : "time");
        return;
      }
      publishStop();
    ...
    bool center_ok = std::abs(o.e_center) < lat_tol_;
    bool axis_ok = std::abs(o.e_axis) < yaw_tol_;
    if (!center_ok) {vy = kp_lat_ * o.e_center;}  // 중앙정렬
    if (!axis_ok) {wz = kp_yaw_ * o.e_axis;}       // z축 정렬

    if (state_ == State::ALIGN) {
      if (center_ok && axis_ok) {  // 정면 정렬 완료 → 전진 단계로
        state_ = State::APPROACH;
        RCLCPP_INFO(get_logger(), "정면 정렬 완료 → APPROACH (z=%.3fm)", o.z_cam);
      }
      // vx = 0 (ALIGN 은 전진하지 않음)
    } else {  // APPROACH
      if (std::abs(o.z_cam - final_distance_) < dist_tol_) {
        publishStop();
        state_ = State::ARRIVED;
        RCLCPP_INFO(get_logger(), "도착 완료(ARRIVED): z=%.3fm (목표 %.3fm)",
          o.z_cam, final_distance_);
        return;
      }
      vx = kp_fwd_ * (o.z_cam - final_distance_);
    }
    ...
    RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000,
      ...
      o.z_cam, o.e_center, rad2deg(o.e_axis), last_vx_, last_vy_, last_wz_);
  }
  ...
  void publishSmoothed(double vx, double vy, double wz)
  {
    ...
    if (sp > max_lin_ && sp > 0.0) {
      double s = max_lin_ / sp;
      vx *= s; vy *= s;
    }
    wz = std::clamp(wz, -max_yaw_, max_yaw_);

    // 2) 최소속도 floor: 0 이 아닌 작은 선속도는 min_lin 까지 끌어올림
    //    (데드밴드로 0 을 만들면 게인×tolerance 궁합에 따라 도착 전에 멈춰버림 → floor 로 해결)
    sp = std::hypot(vx, vy);
    if (sp > 1e-6 && sp < min_lin_) {
      double s = min_lin_ / sp;
      vx *= s; vy *= s;
    }

    // 3) 가속 제한(slew): 직전 발행값 대비 변화량 제한
    double dt = last_dt_ > 0.0 ? last_dt_ : (1.0 / 20.0);
    if (have_prev_) {
      double dv_max = max_lin_acc_ * dt;
      double dw_max = max_yaw_acc_ * dt;
      vx = last_vx_ + std::clamp(vx - last_vx_, -dv_max, dv_max);
      vy = last_vy_ + std::clamp(vy - last_vy_, -dv_max, dv_max);
      wz = last_wz_ + std::clamp(wz - last_wz_, -dw_max, dw_max);
    }
  ...
  void publishStop()
  {
    geometry_msgs::msg::Twist cmd;  // 전부 0
    pub_cmd_->publish(cmd);
  ...
  bool use_compressed_{true}, auto_start_{false};
  int target_id_{1};
  ...
  bool openloop_enabled_{true}, use_odom_{true};
